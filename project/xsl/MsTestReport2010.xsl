...
-->       
 ...
 <xsl:template match="*[local-name()='UnitTestResult']">
		...
		<xsl:variable name="tstClassname" select="/cruisecontrol/build/*[local-name()='TestRun']/*[local-name()='TestDefinitions']/*[local-name()='UnitTest'][@id=$testid]/*[local-name()='TestMethod']/@className" />
		<xsl:variable name="cn" select="substring-before($tstClassname,',')" />

...
<div class="{$colorClass} fitheader">
	...
	<xsl:value-of select="$fit.correctpagecount"/> right, 
	
	<xsl:value-of select="$fit.wrongpagecount"/> wrong,
	
	<xsl:value-of select="$fit.ignorespagecount"/> ignored, 
	
	<xsl:value-of select="$fit.exceptionspagecount"/> exceptions
...
<xsl:for-each select="$fit.result.list">
	...
	<div class="line">
		...
		</input>&#160;<xsl:value-of select="$test.name"/>

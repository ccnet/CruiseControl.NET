...
<xsl:stylesheet
    ...
    <xsl:template match="/">
   	...
   	<xsl:if test="count($messages) > 0">   	
		<xsl:variable name="error.messages" select="$messages[(contains(text(), ' error') and not(contains(text(), '0 error'))) or @level='Error']" />
	        <xsl:variable name="error.messages.count" select="count($error.messages)" />
	        <xsl:variable name="warning.messages" select="$messages[(contains(text(), ' warning') and not(contains(text(), '0 warning'))) or @level='Warning']" />

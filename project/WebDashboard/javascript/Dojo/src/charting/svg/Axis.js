...
if (dojo.render.svg.capable) {
	dojo.extend(dojo.charting.Axis, {renderLines:function (plotArea, plot, plane) {
		...
		function createLabel(label, x, y, textSize, anchor) {
			...
			var textStyle = "text-anchor:" + anchor + ";font-family:sans-serif;font-size:" + textSize + "px;fill:#000;";
...
//			text.appendChild(document.createTextNode(label));
			...
			//Added by ER: Introduce line breaks using the tspan element
			var textParts = label.split("\n");
			
			for (var i = 0; i < textParts.length; i++)
			{
				var labelLineTextSpan = document.createElementNS(dojo.svg.xmlns.svg, "tspan");
				labelLineTextSpan.setAttribute("x", x);
				labelLineTextSpan.setAttribute("y", y + i * 10);
				labelLineTextSpan.appendChild(document.createTextNode(textParts[i]));
				
				text.appendChild(labelLineTextSpan);
			}
			//------------- End of modification ------------

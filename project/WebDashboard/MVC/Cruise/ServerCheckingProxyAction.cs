...
namespace ThoughtWorks.CruiseControl.WebDashboard.MVC.Cruise
{
	public class ServerCheckingProxyAction : ICruiseAction
	{
		...
		public IResponse Execute(ICruiseRequest cruiseRequest)
		{
			...
			{
				return errorViewBuilder.BuildView(string.Format(System.Globalization.CultureInfo.CurrentCulture,"Error - Action [{0}] expects Server to be specified in request", proxiedAction.GetType().FullName));
			}
			else
			{

...
namespace ThoughtWorks.CruiseControl.UnitTests.Core.Tasks
{
	...
	public class MsBuildTaskTest : ProcessExecutorTestFixtureBase
	{
		...
		public void AddQuotesAroundPropertiesWithSpaces()
		{
			...
			string expectedProperties = string.Format(@"/p:CCNetArtifactDirectory={2};CCNetBuildCondition=IfModificationExists;CCNetBuildDate={0};CCNetBuildTime={1};CCNetIntegrationStatus=Success;CCNetLabel=My Label;CCNetLastIntegrationStatus=Unknown;CCNetNumericLabel=0;CCNetProject=test;CCNetRequestSource=foo;CCNetWorkingDirectory=c:\source\",testDateString, testTimeString, result.ArtifactDirectory);
	...
	<logger>Kobush.Build.Logging.XmlLogger,Kobush.MSBuild.dll;buildresult.xml</logger>
		...
		private string IntegrationProperties()
		{
			...
			// Tests that look for the correct arguments will fail if the following properties
			// are not sorted alphabetically.
			return string.Format(@"/p:CCNetArtifactDirectory={3};CCNetBuildCondition=IfModificationExists;CCNetBuildDate={1};CCNetBuildTime={2};CCNetIntegrationStatus=Success;CCNetLabel=1.0;CCNetLastIntegrationStatus=Unknown;CCNetNumericLabel=0;CCNetProject=test;CCNetRequestSource=foo;CCNetWorkingDirectory={0}", DefaultWorkingDirectory, testDateString, testTimeString, result.ArtifactDirectory);
		}
	}
}